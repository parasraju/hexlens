...
${report.metadata.imphash ? `<tr><td>Imphash</td><td class="hash">${report.metadata.imphash}</td></tr>` : ''}
</table>

${report.metadata.assembly ? `
<h2>Assembly (CLR)</h2>
${(() => {
  const a = report.metadata.assembly as Record<string, unknown>
  return `<table>
  ${Object.entries(a).map(([k, v]) => `<tr><td>${escapeHtml(k)}</td><td>${escapeHtml(String(v ?? ''))}</td></tr>`).join('\n')}
  </table>`
})()}` : ''}

${report.resources.length > 0 ? `
<h2>Resources (${report.resources.length})</h2>
<table>
<tr><th>Name</th><th>Type</th><th>Size</th></tr>
${report.resources.map(r => `<tr><td>${escapeHtml(r.name)}</td><td>${escapeHtml(r.type)}</td><td>${r.size.toLocaleString()}</td></tr>`).join('\n')}
</table>` : ''}
...
<footer>Generated by HexLens v0.2.0</footer>
</div>
</body>
</html>`
}
